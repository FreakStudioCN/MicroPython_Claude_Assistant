...
panel 形态角色基于 `Character` 基类，换形象需：

1. 新建文件继承 `Character`，实现 `build()` 和 `tick()` 方法
2. 修改 `device/display_renderer.py` 顶部的 import 一行：

```python
from character import ClaudeCharacter   # 默认 Claude Logo 角色
# 改为你自己的角色类，例如：
from my_character import MyCharacter as ClaudeCharacter
```
...
新建一个继承 `Character` 的类文件，实现 `build()` 和 `tick()` 方法，然后修改 `device/display_renderer.py` 顶部的 import：

```python
# 将此行
from character import ClaudeCharacter
# 改为你自己的角色类
from my_character import MyCharacter as ClaudeCharacter
```
...
| v0.9.3 | 2026-05-25 | 面板版 Sessions 面板支持长按 Tab 清除历史消息；分析并记录 session slot 漂移 bug（待修复） |
