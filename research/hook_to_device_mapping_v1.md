...
| v1.4 | 2026-05-18 | v5 纯展示模式落地（v0.9.0 MVP）：设备→PC 通信完全删除，`m` 字段从 ≤16字扩展至 ≤60字，"已知限制"第1条（多session审批路由）随审批删除而消除，`n` 字段随审批决策消息一并删除，P状态保留为纯提醒用途（灯光+语音），不再携带 `t`/`h` 字段 |

---

## v5 wire 协议参考（当前实现，2026-05-18）

### PC → 设备字段表

| 字段 | 类型 | 含义 | 长度限制 | 必填 |
|------|------|------|---------|------|
| `ss` | array | 所有活跃 session 数组 | - | ✓ |
| `s`  | str  | 状态枚举（见下表） | 1字符 | ✓ |
| `m`  | str  | 工具描述（仅 W 状态） | ≤60字符 | 条件 |
...
|------|------|------|-----------|
| Idle | `{"ss":[{"s":"I"}]}` | 21B | 2 |
| Working | `{"ss":[{"s":"W","m":"Read: main.py"}]}` | 50B | 3 |
| Working (长) | `{"ss":[{"s":"W","m":"Bash: pytest tests/ -v --tb=short"}]}` | 68B | 4 |
| 3 sessions | `{"ss":[{"s":"W","m":"..."},{"s":"C"},{"s":"I"}]}` | ~95B | 5 |
