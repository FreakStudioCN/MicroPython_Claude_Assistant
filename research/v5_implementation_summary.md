...
**实施日期**: 2026-05-06（PC端）/ 2026-05-18（设备端，v0.9.0 MVP）
**版本**: v0.9.0 MVP  
**状态**: ✅ PC端完成 + ✅ 设备端完成（双形态：panel / clock）
...
| 形态 | 硬件 | 状态 |
|------|------|------|
| panel | ESP32 + ST7789 2.4寸屏 + LVGL | ✅ 完成 |
| clock | ESP32C3 + WS2812×2 + I2S扬声器 | ✅ 完成 |

### clock 形态新增功能
- machine.Timer(0) 硬件定时器 ISR 驱动 WS2812，解决灯光与asyncio不同步
- C/E/P 灯光队列：FIFO + 同类型去重，最少显示1s，不打断
- 语音队列（豆包TTS PCM）：与灯光队列行为完全对齐
- 开机自报：startup语音"主人你好，我是码克助手"，未连接时循环播disconnect提示
- 连接白色闪烁1.5s / 断线橙红渐灭1.5s
- flash_device.py：COM口自动扫描、wipe、mpy-cross编译、MAC自动命名

### 设备端协议实现
- 无审批UI、无pong响应、无双向通信
- protocol.py 纯解析 `{"ss":[...]}` wire格式
- 两种renderer共享同一transport/protocol层，variant参数区分形态
...
✅ **PC 端核心功能**：删除设备审批、终端审批、删除心跳、简化状态机、更新测试

✅ **设备端 panel 形态**：LVGL屏幕、小人动画、多session tabs、跑马灯

✅ **设备端 clock 形态**：WS2812灯光队列、I2S语音队列、开机自报、连接/断线效果

✅ **工具链**：flash_device.py烧录脚本、gen_voice_assets.py TTS生成GUI

### 待完成

⏸️ **量产测试**：多设备并发压测、长时间稳定性验证

⏸️ **panel端语音**：clock有语音，panel暂无

⏸️ **测试更新**：test_e2e_stub.py / test_protocol.py / test_daemon_concurrency.py
...
### 短期
1. 量产测试：多设备并发、长时间稳定性
2. 更新 test_e2e_stub.py / test_protocol.py
...
**实施完成日期**: 2026-05-18（v0.9.0 MVP）
**实施人员**: Claude (Sonnet 4.6)
**版本**: v0.9.0 MVP（双形态可用，未量产测试）
