...
文件命名：{log_dir}/{prefix}_0.log, {prefix}_1.log, ...
轮转策略：超过 lines_per_file 行时切换到下一个文件（循环）

MicroPython 端：调用 install() 接入，内部 monkey-patch Logger.log
CPython 端：RotatingFileHandler 继承 logging.Handler，正常 addHandler 接入
"""
...
class _RotatingFile:
    """底层轮转文件，负责写入和轮转，MicroPython/CPython 通用"""
...
if _IS_MICROPYTHON:
    def install(log_dir, max_files=4, lines_per_file=150, prefix="run",
        ...
        """MicroPython 端：monkey-patch Logger.log，将日志重定向到轮转文件"""
    ...
    class RotatingFileHandler(logging.Handler):
        """CPython 端：标准 logging.Handler，正常 addHandler 接入"""
